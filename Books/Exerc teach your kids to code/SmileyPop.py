...
BLACK = (0,0,0)
pygame.init()
screen = pygame.display.set_model([800, 600])
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            if pygame.mouse.get_pressed()[0]:               # Regular left mouse button, draw
                mousedown = True
            else pygame.mouse.get_pressed()[2]:             # Right mouse button, pop
                pos = pygame.mouse.get_pos()
                clicked_smileys = [s for s in sprite_list if s.rect.collidepoint(pos)]
                #If a sprite s in the group sprite_list has a rectangle that collides with the point pos,
                # group it together as a list [s] and store the list as clicked_smileys
